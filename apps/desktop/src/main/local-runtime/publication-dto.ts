...
import type { PrepareRemoteDraftRequest } from "../publishing/draft-preparation.js";
import type { RemotePublicationAsset } from "../publishing/remote-asset-downloader.js";
...
export function runtimePublicationStatus(summary: PublicationSummary) {
  ...
  const terminal =
    state === "published" || state === "failed" || state === "uncertain";
  return {
...
function runtimeState(state: PublicationSummary["state"]) {
  ...
  if (state === "rejected" || state === "cancelled") return "failed" as const;
  return state;
}
